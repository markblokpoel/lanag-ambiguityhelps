...
GGdistribution <- ggplot(DFcounts) +
  ...
  scale_x_continuous("distribution", breaks = pretty_breaks(1)) +
  scale_fill_manual("distributions", values = c("theoretical distribution" = "#9ecae1", "sample distribution" = "#2b8cbe")) +
  scale_color_manual("distributions", values = c("theoretical distribution" = "#9ecae1", "sample distribution" = "#2b8cbe")) +
  scale_shape_manual("", values = c("theoretical distribution mean" = 5)) +
  ...
  if(grepl("uniform", dataFolder)) {
    GGdistribution <- GGdistribution + geom_rect(data = DFminMax, aes(xmin = 0, xmax = .5,
                                                ymin = mina, ymax = maxa,
                                                fill = "theoretical distribution",
                                                color = "theoretical distribution"))
  }

  GGdistribution <- GGdistribution + geom_rect(aes(xmin = 0, xmax = N,
                             ymin = asymmetry, ymax = asymmetry,
                             fill = "sample distribution",
                             color = "sample distribution"))

  if(grepl("uniform", dataFolder)) {
    GGdistribution <- GGdistribution + geom_point(data = DFminMax, aes(x = 0.125, y = meana, shape = "theoretical distribution mean"))
  }
```
```{r echo = FALSE}
GGdistribution
...
# pdf(file = "random2000_distribution.pdf")
# GGdistribution + theme(legend.position = "bottom")
# dev.off()
# 
