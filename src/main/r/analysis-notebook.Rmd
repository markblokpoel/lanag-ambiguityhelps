...
multiplot(plotCommSuccess(DFstats, DFdelta, 3, 3, axis = TRUE),
...
plotVariance <- function(dfsim, legendPos = "none") {
  ...
  ggplot(DFvariance, aes(x = varBin, y = N, fill = factor(ag))) + ggplot_theme +
    ...
    scale_x_continuous("ambiguity variance", limits = c(0,8), breaks = pretty_breaks(8)) +
    scale_fill_manual("agent", values = c("1" = "#7b3294", "2" = "#008837")) +
    geom_bar(position = "dodge", stat = "identity") +
    facet_wrap(amb ~ ., ncol = 4, labeller = label_bquote(cols= alpha == ~ .(amb)/.(8)))
}
```
*Figure SI.?: Supporting simulation I variance of ambiguity.*
```{r}
...
pdf(file = paste0(outputPrefix, "figsi5_variance.pdf"), height = 3, width = 8)
plotVariance(DFsim, "bottom")
dev.off()
```
...
*Figure SI.?: Supporting simulation II variance of ambiguity.*
```{r}
plotVariance(DFsim, "bottom")
```

```{r results='hide'}
pdf(file = paste0(outputPrefix, "figsi6_structured_overview.pdf"), width = 15, height = 12)
multiplot(plotCommSuccess(DFstats, DFdelta, 3, 3, axis = TRUE),
          plotDeltaAsymmetry(DFdelta),
          plotCommSuccess(DFstats, DFdelta, 4, 4), #3
          plotCommSuccess(DFstats, DFdelta, 4, 5), #4
...
pdf(file = paste0(outputPrefix, "figsi5_structured_replication.pdf"), height = 4, width = 8)
plotReplication(DFreplication21Summary, DFreplication10Summary, legendPos = "none")
dev.off()

pdf(file = paste0(outputPrefix, "figsi8_structured_distribution.pdf"), height = 5, width = 8)
plotMainSimulationDistribution(computeDistributionStatistics(DFsim), theoreticalLims = FALSE, legendPos = "right")
dev.off()

pdf(file = paste0(outputPrefix, "figsi9_variance.pdf"), height = 3, width = 8)
plotVariance(DFsim, "bottom")
dev.off()
```
